...
def _simulate_portfolio(
    ...
    *,
    ...
    # Ensure that signals generated at time t are only executed at t+1.
    # This mathematically guarantees zero look-ahead bias at the execution layer.
    if isinstance(target_weights, pd.DataFrame):
        dynamic_targets = target_weights.reindex(columns=syms).clip(lower=0.0).fillna(0.0)
        # The Guard: shift targets forward by 1 bar. Signals from close(t)
        # become the target holding for bar t+1.
        dynamic_targets = dynamic_targets.shift(1).fillna(0.0)
    ...
    held = dynamic_targets.iloc[0].copy() if is_dynamic else static_target.copy()
    ...
    for i in range(n_bars):
        ...
        drifted = new_value / new_total if new_total > 0 else held

        target = dynamic_targets.iloc[i] if is_dynamic else static_target
