...
def _simulate_portfolio(
    ...
    *,
    ...
    for i in range(n_bars):
        ...
        ret_i = returns_arr[i]
        r_t = float(np.sum(ret_i * held))

        new_value = held * (1.0 + ret_i)
        new_total = float(np.sum(new_value))
        drifted = new_value / new_total if new_total > 0 else held

        target = dynamic_targets_arr[i] if is_dynamic else static_target_arr
        ...
        if should_rebalance:
            turnover = float(np.sum(np.abs(drifted - target)))
